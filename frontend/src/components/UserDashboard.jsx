...
function UserDashboard() {
  ...
  return (
    <div className='w-screen min-h-screen flex flex-col gap-5 items-center bg-[#fff9f6] overflow-y-auto'>
      ...
      {/* Restaurants — Swiggy style */}
      <div className='w-full max-w-6xl flex flex-col gap-5 items-start p-[10px]'>
        <h1 className='text-gray-800 text-2xl sm:text-3xl font-bold'>Restaurants to explore</h1>
        <p className='text-gray-400 text-sm -mt-3'>in {currentCity}</p>
        <div className='w-full relative'>
          {showLeftShopButton && (
            <button className='absolute left-0 top-1/2 -translate-y-1/2 bg-white border shadow-md text-gray-700 p-2 rounded-full z-10 hover:bg-gray-50' onClick={() => scrollHandler(shopScrollRef, "left")}>
              <FaCircleChevronLeft />
            </button>
          )}
          <div className='w-full flex overflow-x-auto gap-5 pb-2 scrollbar-hide' ref={shopScrollRef}>
          ...
          {showRightShopButton && (
            <button className='absolute right-0 top-1/2 -translate-y-1/2 bg-white border shadow-md text-gray-700 p-2 rounded-full z-10 hover:bg-gray-50' onClick={() => scrollHandler(shopScrollRef, "right")}>
              <FaCircleChevronRight />
            </button>
          )}
        </div>
